...
class FileTransport extends egg_logger_1.Transport {
    ...
    renameLogFile(filename) {
        ...
        const refilename = path_1.default.join(dir, `${today.format('YYYY-MM-DD')}-${mill}.log`);
        fs_1.default.renameSync(filename, refilename);
    }
    /**
     * 检查当前的日志文件是否为当天
     */
    ...
    getPresentFilename() {
        ...
        const today = dayjs_1.default();
        const ddir = path_1.default.join(dir, today.format('YYYY-MM'));
        const dfilename = today.format('YYYY-MM-DD');
        const filename = path_1.default.join(ddir, `${dfilename}.log`);
        return filename;
    }
